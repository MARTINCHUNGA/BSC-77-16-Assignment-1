...
class CornersProblem(search.SearchProblem):
    ...
    def getSuccessors(self, state):
        ...
        for action in [Directions.NORTH, Directions.SOUTH, Directions.EAST, Directions.WEST]:
            ...
            dx, dy = Actions.directionToVector(action)
            nextx, nexty = int(x + dx), int(y + dy)
            hitsWall = self.walls[nextx][nexty] #make sure pacman does not hit the wall
...
def cornersHeuristic(state, problem):
    ...
    unvisitedCorners = [] # store not visited corners
    for i in range(len(corners_not_visited)):
        if not corners_not_visited[i]:
            unvisitedCorners.append(corners[i])

    
    if len(unvisitedCorners) > 0:
        closestPoint = findClosestPoint(current_location, unvisitedCorners)
        farthestPoint = findFarthestPoint(current_location, unvisitedCorners)# distance btwn current node and to all corner nodes
        ...
        currentToClosest = mazeDistance(current_location, closestNode, currentNode) #distance btwn current location and closest manhattan node
...
def foodHeuristic(state, problem):
    ...
    if len(foodList) > 0:  #distance btwn food nodes and current node
        ...
        currentToClosest = mazeDistance(position, closestFoodNode, currentNode) #distance btwn current location and closest manhattan node
